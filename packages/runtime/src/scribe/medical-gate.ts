...
const FORBIDDEN_CONDITION = /\b(?:you(?:'re| are)?\s+(?:have|has|may have|might have|could have|likely have|probably have|suffer(?:ing)? from|showing signs of)|have|has|shows?|showing|indicates?|means?|proves?|suggests?|signs? of|symptoms? of|consistent with|diagnosed with)\s+(?:[a-z-]+\s+){0,2}?(?:anxiety|depression|atrial fibrillation|afib|sleep apnea|hypertension|diabetes|heart disease|hypoxia|tachycardia|insomnia|asthma|adhd|ptsd|ocd|bipolar|covid|flu|migraine|cancer)\b/i;
const DIAGNOSIS_OR_RISK = /\b(?:you (?:are at risk(?: for| of)?|are stressed)|(?:data|reading|readings|pulse|hrv|sleep|blood pressure|glucose)\s+(?:shows?|proves?|means?|indicates?|suggests?)\s+(?:that\s+)?(?:you\s+)?(?:have\s+)?(?:a\s+)?(?:medical|condition|disease|disorder|illness))\b/i;
...
const IMMUTABLE_NEVER_CLAIM = /\b(?:this (?:is|may be|might be|could be|appears to be|seems to be) (?:a\s+)?symptom of|you should see (?:a|an|your)?\s*[a-z-]+(?:\s+[a-z-]+){0,2}\s+because|based on your data,?\s+you (?:(?:could|may|might) be|are)\s+at risk(?: for)?|your (?:hrv|heart rate|sleep|blood pressure|glucose) indicates?\s+[a-z-]+(?:\s+[a-z-]+){0,3})\b/i;
