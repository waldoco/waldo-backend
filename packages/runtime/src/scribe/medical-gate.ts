...
const IMMUTABLE_NEVER_CLAIM = /\b(?:this is (?:a\s+)?symptom of|you should see (?:a|an|your)?\s*[a-z-]+(?:\s+[a-z-]+){0,2}\s+because|based on your data,?\s+you (?:(?:could|may|might) be|are)\s+at risk(?: for)?|your (?:hrv|heart rate|sleep|blood pressure|glucose) indicates?\s+[a-z-]+(?:\s+[a-z-]+){0,3})\b/i;
const WORD_DOSAGE = /\b(?:take|start|increase|decrease|use)\b[^.!?]{0,40}\b\d+(?:\.\d+)?\s*(?:milligrams?|micrograms?|grams?|milliliters?)\b/i;
