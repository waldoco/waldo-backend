...
export const approvalDesk = (sql: SqlStorage, deps: Readonly<{
  ...
  const apply = async (client: GoogleClient, p: ProposeCalendarChangeArgs): Promise<Undo | null> => {
    ...
    if (p.action === 'move') {
      await client.moveEvent(p.event_id!, p.start!, p.end!);
      return { op: 'move', id: p.event_id!, start: before.start, end: before.end };
    }
    await client.cancelEvent(p.event_id!);
    return null;
  };
  ...
  return {
    async propose(p) {
      ...
      sql.exec("INSERT INTO ledger (id, kind, status, summary, payload_json, undo_json, created_at, decided_at) VALUES (?, 'calendar_change', 'open', ?, ?, NULL, ?, NULL)", id, summary, JSON.stringify(p), deps.now());
    ...
    async callback(query, trace) {
      ...
      const proposal = JSON.parse(entry.payload_json) as ProposeCalendarChangeArgs;
      try {
        if (action === 's') {
