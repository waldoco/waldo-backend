...
export const callMcp = async (server: McpServerConfig, tool: string, args: Record<string, unknown>, fetcher: typeof fetch = fetch): Promise<unknown> => {
  ...
  const text = await called.response.text();
  ...
  const payload = text.startsWith('event:') || text.includes('\ndata:')
    ...
    : text;
  const parsed = JSON.parse(payload) as { result?: { content?: unknown; isError?: boolean }; error?: { message?: string } };
  if (parsed.error) throw new Error(`mcp error: ${parsed.error.message ?? 'unknown'}`);
  const result = parsed.result ?? {};
  if (result.isError) throw new Error(`mcp tool error: ${JSON.stringify(result.content).slice(0, 200)}`);
...
export const callMcpToolHandler = (serversRaw: string | undefined): ToolHandler<CallMcpToolArgs, unknown, ToolDispatcherContext> => ({
  ...
  handle: async ({ server, tool, args }: CallMcpToolArgs): Promise<ToolResult<unknown>> => {
    ...
    try {
      return { ok: true, data: { output: await callMcp(found, tool, args), source_taint: 'external' as const }, source_taint: 'external' };
    } catch (error) {
