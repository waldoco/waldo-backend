...
describe('reminder book on the Telegram owner object', () => {
  it('schedules, lists, fires and cancels reminders beside chat memory', async () => {
    ...
    await runInDurableObject(stub, async (_instance, state) => {
      ...
      const once = await book.set({ note: 'call mom', at: '2026-09-23T18:30', repeat: 'none' });
      const daily = await book.set({ note: 'drink water', at: '2026-09-23T09:00', repeat: 'daily' });
      await expect(book.set({ note: 'late', at: '2026-09-23T10:00', repeat: 'none' })).rejects.toThrow('already past');
      expect(daily.at).toBe('2026-09-24T09:00');
      expect(book.list().map((r) => [r.note, r.at, r.repeat])).toEqual([['call mom', '2026-09-23T18:30', 'none'], ['drink water', '2026-09-24T09:00', 'daily']]);
      expect(scheduler.read(once.id)?.due_at).toBe(Date.parse('2026-09-23T13:00:00Z'));
      expect(await state.storage.getAlarm()).toBe(Date.parse('2026-09-23T13:00:00Z'));
