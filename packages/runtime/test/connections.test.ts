...
describe('incremental Google access', () => {
  it('a mail tool on a calendar-only grant reports a scope_missing intent for mail, not a retry and not a URL', async () => {
    ...
    const draft = googleHandlers(google, { propose: async () => 'p', proposeSendEmail: async () => ({ id: 'p', reused: null }), record: () => undefined }, { timezone: 'UTC', now: () => new Date() }).find((tool) => tool.name === 'draft_email')!;
