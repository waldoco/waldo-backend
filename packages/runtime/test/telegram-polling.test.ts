...
describe('TelegramPollingAdapter', () => {
  ...
  it('accepts photos and documents with their caption and keeps voice notes unsupported', async () => {
    ...
    const adapter = new TelegramPollingAdapter({
      async getUpdates() {
        return [
          ...
          { update_id: 3, message: { ...owner, voice: { file_id: 'v', duration: 3 } } },
        ];
      },
    }, 1);
    const polled = await adapter.poll(0);
    expect(polled.accepted).toEqual([
    ...
    expect(polled.unsupported).toEqual([{ updateId: 3, messageId: null, senderId: 7, chatId: 7 }]);
  });
