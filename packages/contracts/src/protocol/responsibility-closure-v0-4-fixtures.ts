...
export function buildResponsibilityClosureV04Bundle(hashHex: (value: string) => string): Record<string, string> {
  ...
  const verification = verificationV04Schema.parse({ protocolVersion: '0.4', id: 'verification_fixture', ownerId: 'owner_fixture', revision: 1, outcome: { id: 'outcome_fixture', revision: 1 }, acceptanceCheck: { id: 'check_fixture', revision: 1, digest }, evidence: [{ id: evidence.id, revision: 1, digest }], evidenceSetDigest: digest, verifier: { id: 'verifier_fixture', version: '1.0.0', independentFromProducer: true, disclosureRef: 'disclosure_fixture' }, methodVersion: '1.0.0', state: 'indeterminate', findingsRef: null, findingsDigest: null, verifiedAt: '2026-08-13T12:02:00.000Z' });
  ...
  bundle['closure.rejections.json'] = file({ protocolVersion: '0.4', cases: [{ name: 'provider-self-acceptance', value: { ...candidate, acceptance: 'accepted' } }, { name: 'unavailable-as-passed', value: { ...verification, state: 'passed', verifier: { ...verification.verifier, independentFromProducer: false } } }, { name: 'inline-evidence', value: { ...candidate, inlineEvidence: { secret: true } } }] });
