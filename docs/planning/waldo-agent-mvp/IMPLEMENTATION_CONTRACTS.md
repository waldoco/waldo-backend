...
**Owner decision, 2026-09-23 (staging, owner-only).** The staging OAuth client requests the full workspace set up front so new tools don't force re-consent: openid, email, `calendar.events`, `calendar.readonly`, `gmail.modify`, `gmail.compose`, `gmail.send`, `tasks`, `drive`, `documents`, `spreadsheets`, `presentations`, `contacts`. The consent screen stays in Testing with the owner as the only test user. Scope is not permission: each tool keeps its own approval gate, and today no tool reads mail, sends mail, or touches Drive/Docs/Sheets/Slides/Tasks/Contacts. Before any non-test user can connect with this set, Google app verification (sensitive + restricted scopes, including the security assessment above) must be complete; otherwise trim the set to what shipped tools use.
