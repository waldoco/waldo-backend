...
| --- | --- | --- |
| `waldo-backend` | product-boundary evidence `dd434e9`; B0 implementation `c37956a`; B1 v0.4 contract release `df0abae` / fixture tree `adff7e5`; #80 sole-writer runtime reviewed at `eebe931` and landed at `cffae3b` with identical tree `43aa5b9`; #87 port/fake reviewed at `5b3d9f5` and landed at `origin/main@9a2b11b` with identical tree `8a202c5`, checked 2026-08-14 | These SHAs prove baseline fixtures, migration allocation lineage, route/OpenAPI parity, the local verification wall, test-harness alarm isolation, `contract_defined` status for the released v0.4 families, the one landed canonical ExecutionRequest/Attempt/Session/Lease/Observation/cancellation/reconciliation writer, and landed deterministic backend contract/fake conformance around its execution-environment boundary. They do not prove `adapter_conformance_passed`, #88 public routing or durable production intent composition, real adapters, live integration, closure runtime, deployment, or production. Re-pin before later implementation claims. |
