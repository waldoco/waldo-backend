...
| --- | --- | --- |
| `waldo-backend` | product-boundary evidence `dd434e9`; B0 implementation `c37956a`; B1 v0.4 contract release reviewed at `df0abae` and landed at `origin/main@7067f15` with an identical tree, checked 2026-08-14 | The later SHAs prove baseline fixtures, migration allocation lineage, route/OpenAPI parity, the full local verification wall, test-harness alarm isolation, and `contract_defined` status for the released presence/channel, execution, acceptance-check, judgment-authority, closure, continuity, effect and obligation-context families. They do not prove the #80 execution writer, adapter conformance, closure runtime, live integration, deployment, or production. Re-pin before later implementation claims. |
