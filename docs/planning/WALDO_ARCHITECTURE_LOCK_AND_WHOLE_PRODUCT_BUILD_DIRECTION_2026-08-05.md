...
**B2 evidence refresh:** 2026-08-16; #82 JudgmentAuthority runtime is landed at `origin/main@105e4b5`; #84's v0.6 closure contract remains an open, unmerged candidate in PR #133
**Personal-understanding refresh:** 2026-08-17; the locked context boundary now names admitted-source Profile compilation, local/Waldo kernel ownership, provider-specific projection, record-scoped Kennel attachment, and universal external-LLM egress
**Scope:** Waldo backend agent, Kennel desktop presence/executor/local memory, mobile Health/Care presence, messaging presences, Personal Profile and continuity, governed external-model egress, execution, workspace, connectors, and distribution contracts
...
**[Decision — locked] Online backend authority is required for the current product.** Protocol/capability version 0.1 advertises `offlineCommands: "none"`. While disconnected, Kennel, mobile, and every other presence cannot create or queue Waldo commands, submit Waldo approval, mint authority, execute Waldo Outcome work, change canonical Waldo state, or claim completion. A presence may display an explicitly stale last-synced projection as read-only presentation; that is not offline Waldo functionality. Kennel's separate no-account Local Memory Core may compile, inspect, correct, retrieve, export, and delete its own pre-attachment/`local_only` records offline; those operations create no Waldo Outcome, authority, Acceptance, Open Loop, or cloud claim. Local-LLM support may be added later as an online-governed Waldo provider/executor adapter; it does not create offline Waldo authority or an offline truth fork.
...
**[Observed at landed `waldo-backend@105e4b5`]** Responsibility capture v0.1/v0.2, bounded planning-turn v0.3, authenticated public ingress, owner routing, `IdentityPresenceModule`, `WaldoCoordinator`, `OutcomeModule`, owner events/projections, and one leased/fenced/cancellable zero-tool planning turn are implemented. B1 adds released v0.4 responsibility contracts, `PlanningExecutionModule` as the canonical writer for execution request/attempt/session/lease/observation/reconciliation state, one recover-before-issue execution-environment seam, and one strict authenticated start-only WorkUnit bridge through the existing trusted RunLoop composition. B2 #82 adds the v0.5 `JudgmentAuthorityModule` sole writer, authenticated answer/handoff ingress, ordered Needs You projection, replay/rebuild, and rollback proof without grant consumption or external I/O. The reviewed #131 tree and landed merge tree are identical. The trusted RunLoop, ContextComposer, DeliveryGate, scheduling, journal/outbox, safety, and effect foundations remain the physical execution/I/O substrate.

**[Not yet proved]** The public bridge is default-disabled and has deterministic local/fake proof only; non-local composition has no real execution-environment adapter. Evidence/Verification/Acceptance/OpenLoop/ReEntry reducers, automatic Personal Profile compilation, the local kernel/provider-projection boundary, universal external-LLM egress, governed Kennel memory attachment, multi-presence/Home/channel synchronization, service-first Connections, real Calendar/inbox product effects, Telegram/Discord adapters, reviewed routine promotion, persistent cloud execution, Electron Kennel consumption, mobile acceptance, staging, and production proof remain missing or unproved. #84's v0.6 contract is unmerged PR evidence, not runtime truth. Resume/steer/pause intent durability and upgrade of a WorkUnit already occupying the v0.3 planning row remain architecture stops. The capability matrix carries the row-level status; this lock must not be cited as implementation proof.

**[Observed 2026-08-16 after #82]** PRs #130 and #131 landed the additive v0.5 Judgment contracts and sole JudgmentAuthority runtime. At the fresh-main #131 wall, contracts passed 71 files / 1,572 tests, runtime passed 43 files / 1,152 tests, integration passed 2 files / 5 tests, eight Supabase migrations plus 53 pgTAP assertions passed, and QA/Security/Standards/Spec passed. This proves #82 at `module_implemented`; it does not prove grant consumption, #84 closure runtime, #85 continuity, an adapter, staging, deployment, or production behavior. PR #133 is a reviewed but unmerged contract-only candidate and requires separate authorization.
...
- one editable Personal Profile continuously compiled from `SourceAdmissionPolicy`-admitted work/personal evidence—including allowed sent mail—with explicit statements/corrections above inference;
- one compact current `PersonalUnderstandingKernel` for eligible local/Waldo sessions and separately governed destination projections for every external provider call;
...
1. Waldo backend owns identity binding, canonical product state, admitted `waldo_synced` Profile/Memory Claim revisions, owner-root kernel/context policy, authority, acceptance, Open Loop closure, and ordered projections.
2. Kennel is Waldo's Electron desktop presence, standalone Local Memory Core, and local executor with synchronized Home and Xirp-informed Work. **Kennel proposes; the owner Durable Object admits.** Kennel owns pre-attachment and `local_only` Profile/Memory Claims, local source cursors/indexes/kernel/provider-use audit, local operation durability, and workspace processes; it never owns canonical Waldo Outcome, authority, admitted `waldo_synced` records, acceptance, or closure.
...
- **[Decision — locked · Adopt]** Kennel may operate a useful no-account Local Memory Core and automatic Personal Profile Compiler. Before attachment and for `local_only`, Kennel is sole writer. Attachment begins with content-free inventory and record-scoped receipts; after admission, `ContinuityModule` is sole writer for each `waldo_synced` claim while Kennel retains local source custody.
...
| Admit connected sources and enforce account/category/organization/processor policy | `CapabilityRegistry` + `SourceAdmissionPolicy` |
| Own admitted Profile/Memory Claim corrections, tombstones, and record-scoped attachment | `ContinuityModule` |
| Compile minimum purpose-bound context | `ContextCompiler` |
| Gate every external model call and record its destination-specific disclosure | `PostureModule` + `SourceAttributionLedger` |
...
- A service-first Connection binds a recognizable account to declared read/write capabilities, eligible purposes, Profile-compilation categories, storage/processing location, remote-model destinations, processor region/retention, approval policy, credential-handle metadata, freshness, usage receipts, and revocation. OAuth, MCP, CLI, browser login, or native-adapter mechanics remain behind the Interface. A user connection cannot override organization/employer denial, lawful-access rules, third-party/confidential/sensitive exclusion, or unsupported processor policy.
...
| ProfileClaim, MemoryClaim, ContextClaim, Spot, Episode, Constellation, OpenLoop, ReEntryPoint | `ContinuityModule` | Per-owner DO SQLite | `SourceAdmissionPolicy`-admitted provenance-bearing proposal, explicit statement/correction, record-scoped attachment receipt, or authorized lifecycle command |
...
| Capability manifests, per-owner eligibility, `ConnectionBinding`, SourceAdmissionDecision, and source coverage/cursor lifecycle | `CapabilityRegistry` | Versioned registry plus per-owner connection/admission/policy state; raw source bodies stay at declared custody and secret values stay in broker/vault | Authenticated service binding, account/category/organization/processor policy inputs, or source-pinned manifest and conformance result |
| PersonalUnderstandingKernel, ContextProjection, ProviderContextProjection, and projection recipes | `ContextCompiler` | Per-owner DO SQLite plus source references and content-minimized digests | Purpose-bound compilation command over current admitted claims and attributable source observations |
...
| ContextUse, SourceUsageReceipt, AttributionEvent | `SourceAttributionLedger` | Per-owner DO SQLite | Purpose-bound local/remote use observation including source/kernel/projection digests, categories/omissions, consumer/provider/model/destination, policy/sanitizer generations, retention, expiry, and result |
...
| ExecutionPosture, ExternalLLMEgressDecision, and containment disposition | `PostureModule` | Bounded security ledger | Redacted executor/egress/credential/process observation; remote-model admission requires destination/data-class consent, active applicable DPA, remote-egress policy, region/retention, and fail-closed Scribe result |
...
- `SourceAdmissionPolicy` runs before Profile/Memory compilation. It enforces lawful access, account ownership, organization/employer policy, third-party/confidential/sensitive exclusions, purpose, processor region/retention, and revocation generation. A connected account is not blanket admission and partial source coverage remains visible.
- Only `ContinuityModule` accepts/revises `waldo_synced` Profile/Memory Claims, and only `ContextCompiler` creates the local/Waldo-owned `PersonalUnderstandingKernel`, execution context, or provider projection. Every output is purpose-, audience-, destination-, data-class-, source/profile/kernel-, freshness-, expiry-, deletion-generation-, and digest-bound. Explicit user statements/corrections outrank inference.
- `ExternalLLMEgressGate` is a universal code-owned admission step before planning, execution, Session Brief, retrieval, semantic Verification, embedding, transcription, enrichment, or any other remote model call. It requires explicit destination/data-class consent, an active applicable DPA, `RemoteContextEgressPolicy`, processor region/retention, and fail-closed Scribe masking of PII, secrets, and health values. A provider adapter, model-facing tool, MCP path, fallback route, or recovery path cannot bypass or widen it.
- Work and personal categories remain independently permissioned purpose scopes. The unsanitized kernel stays local or Waldo-owned; an external provider receives only the smallest destination-specific projection. Context and Profile claims never issue authority.
...
Standalone Kennel memory does not weaken this boundary. Before attachment and for `local_only` records, Kennel is sole writer and the backend must not receive even their metadata. Attachment sends a content-free class/count inventory first; only selected accepted records become `waldo_synced` after per-item admission receipts and stable ID mapping. Correction, deletion, detach, index rebuild, restore, and reconnect preserve one writer per record and cannot resurrect excluded/tombstoned material.
...
10. Source connection cannot override employer/organization denial, unlawful access, account/category exclusion, confidential/sensitive data policy, or unsupported processor region/retention; partial failure cannot be represented as complete coverage.
11. Every remote model path denies when destination/data-class consent, applicable DPA, remote-egress policy, region/retention, or Scribe state is missing/stale; the provider receives no unsanitized kernel, unrelated category, raw health, PII, secret, or credential.
12. A Profile correction/exclusion advances generations, marks affected kernels/projections/context uses, rebuilds derived indexes, and survives restart, attachment/detach, backup/restore, and provider-session refresh without resurrection.
...
| Standalone memory and personal understanding | No-account Local Memory Core, source admission, Profile compiler, local kernel, Claude/Codex seam, provider-use audit, correction/export/delete, record-scoped attachment | Local record/source/egress contracts are published; may advance independently of backend Outcome integration | Fresh-session continuity and baseline eval; account/category denial; source partial failure; cross-scope/egress denial; offline restart; attach/detach; non-resurrection |
...
| Owner-root Personal Profile and context | Admitted Profile/Memory Claims, kernel/projection recipes, ContextUse, universal external-LLM egress | Source-admission/continuity/context/egress contracts are published | One writer per record; explicit correction precedence; minimum destination projection; no-bypass/DPA/Scribe conformance; provider-use inspection |
...
12. Kennel proves useful no-account personal understanding across fresh supported Claude/Codex sessions, then attaches selected records through content-free inventory and per-item receipts; one correction/exclusion converges across local and Waldo-owned projections without uploading `local_only` data or resurrecting deleted claims.
13. Every external-model route—including planner, executor, Brief, verifier, embedding, transcription, enrichment, and model-facing tool/MCP paths—receives only its consented Scribe-sanitized destination projection, and a stale DPA/policy/sanitizer condition fails closed while local understanding remains available.
...
**STOP pending fresh human direction.** B0, bounded B1, and #82 JudgmentAuthority are landed at `origin/main@105e4b5`. #84 PR #133 remains open, unmerged, and contract-only; #85 and B3–B6 remain blocked or deferred. Do not merge #133, begin #84 runtime, start #85, or resume the B2–B6 goal until the human explicitly redirects work from the Kennel harness. On resumption, re-pin fresh `origin/main`, #78/#84/#116, and PR #133; merge only under separate exact-SHA authorization, compare reviewed and landed trees, and pass the fresh-main wall before another write lane. The Personal Profile, kernel/projection, universal external-model egress, and attachment contracts remain later B4/B5 impact surfaces, not permission to widen #84 or bypass the current stop.
