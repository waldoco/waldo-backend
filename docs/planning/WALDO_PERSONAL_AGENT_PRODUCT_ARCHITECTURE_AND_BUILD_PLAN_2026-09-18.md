...
Before a route can receive mail, meeting, or derived-health content, its processor/DPA and retention posture must be documented and accepted for that class. Cloudflare AI Gateway payload collection is explicitly disabled (`cf-aig-collect-log-payload: false` or the verified current equivalent), and a synthetic canary proves that prompts/tool output are absent from gateway/provider logs and application traces. If no-training/ZDR/region or log suppression cannot be established, that route receives only the lower data classes its evidence permits.
...
The connector never owns Waldo memory, authority, Outcome truth, or Acceptance. Connector results are untrusted observations until admitted.
