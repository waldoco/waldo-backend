...
**Backend evidence refresh:** 2026-08-15; B0 and bounded B1 are landed at `origin/main@883ef913`; use the [benchmark capability audit](../research/2026-08-15-benchmark-agent-capability-audit.md) for current Pi/Hermes/OpenClaw comparison
...
- **[Observed fact]** The current backend evidence baseline is [`waldo-backend@883ef913`](https://github.com/Pin4sf/waldo-backend/commit/883ef9138df0bdbad70fcbc4d45cfec203d942ad). It includes B0; released v0.4 responsibility contracts; the canonical v0.4 execution writer; the recover-before-issue execution-environment seam; and one authenticated, default-disabled, start-only public WorkUnit path through owner routing, Coordinator admission, #80, the existing trusted RunLoop composition, #87, and bounded observation admission.
- **[Observed fact]** Reviewed #128 head `85e59e6` and landed main have identical tree `2d2b0ec`. The path has deterministic local/fake proof and no real adapter. It does not implement Evidence, Verification, Acceptance, OpenLoop, ReEntry, multi-presence, Connections, real effects, routines/cloud work, deletion, staging, or production acceptance.
...
### 1.3 Backend delivery refresh after B1
...
| Judgment, Evidence, Verification, Acceptance, OpenLoop/ReEntry | Released schemas/fixtures; runtime writers and public product flow absent | `contract_defined` | B2 #82/#84/#85 |
...
**[Observed fact]** Pi, Hermes Agent, OpenClaw, QM, and Think already expose substantial combinations of goals, memory, tools, subagents, channels, background work, recovery, policy, and traces. **[Inference]** B0/B1 do not make Waldo a complete peer. Waldo's credible differentiated claim remains conditional: B2–B6 must prove that independent Evidence, Verification, conscious Acceptance, authority, effect reconciliation, and exact re-entry improve real delegation rather than merely add ceremony.
...
Status uses the 2026-08-15 refresh at `waldo-backend@883ef913`: **shipped**, **partial**, **stub**, **proposed**, **missing**, or **rejected**. Read older row wording with [Section 1.3](#13-backend-delivery-refresh-after-b1), which is authoritative for B0/B1 delivery changes. “Implemented-local” is still not a hosted, cross-surface, or production claim. “Target” means the architecture has a named home for it; no target row is a shipped claim.
