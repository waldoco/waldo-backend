...
- **[Observed fact]** The current backend evidence baseline is [`waldo-backend@dd434e9`](https://github.com/Pin4sf/waldo-backend/commit/dd434e9bb5dedc4a135e43e30571a141599e8991). It includes released responsibility contracts v0.1/v0.2, planning-turn v0.3, an authenticated public Worker adapter, owner routing, `IdentityPresenceModule`, `WaldoCoordinator`, `OutcomeModule`, owner events/projections, and `PlanningExecutionModule` for one durable zero-tool provider planning turn.
- **[Observed fact]** That product path stops at Outcome `captured`, Mission `proposed`, WorkUnit `planned`/`planning_authorized`, and candidate planning. The trusted RunLoop, ContextComposer, DeliveryGate, scheduling, journal/outbox, safety, and effect foundations are substantial but are not joined to product WorkUnit execution, Evidence, Verification, Acceptance, OpenLoop, or ReEntry.
...
Status uses the current local evidence pin `waldo-backend@dd434e9`: **shipped**, **partial**, **stub**, **proposed**, **missing**, or **rejected**. “Implemented-local” is still not a hosted, cross-surface, or production claim. “Target” means the architecture has a named home for it; no target row is a shipped claim.
...
| General Outcomes and planning | Medley mission contract/DAG; Hermes goals/Kanban; Dimension Action Plan | **[Decision — committed]** Outcome → optional Mission → WorkUnits at the current information frontier; not limited to repositories or coding. | **Partial implemented-local:** capture, graph persistence/projection, and one zero-tool planning turn; no executable lifecycle | OutcomeModule + PlanningExecutionModule; depends on execution/evidence bridge |
