...
At implementation pin `origin/main@9a2b11b`, B0 baseline convergence, #81's additive B1 contract/fixture release, #80's sole v0.4 execution writer, and #87's backend execution-environment boundary are landed. [#80](https://github.com/Pin4sf/waldo-backend/issues/80), [#81](https://github.com/Pin4sf/waldo-backend/issues/81), and [#87](https://github.com/Pin4sf/waldo-backend/issues/87) are closed; PRs #125 and #126 are merged. #87 reviewed head `5b3d9f5` and landed merge `9a2b11b` have the identical Git tree `8a202c5`. [#88](https://github.com/Pin4sf/waldo-backend/issues/88) is the next and only active B1 implementation frontier:
...
- active #88 boundary: one authenticated public command may accept only a bounded expected WorkUnit revision as a concurrency precondition, then must bind server-re-read canonical WorkUnit revision/digest, Outcome binding, and authority ceiling through the existing `RunLoopDO` shell to the #80 writer and #87 boundary without exposing caller-supplied owner, provider, environment, context, operation-intent, or Durable Object authority;
- missing or unproved: the #88 bridge and durable production operation-intent composition, every real adapter, canonical Judgment/Evidence/Verification/Acceptance/OpenLoop/ReEntry, multi-presence Home and channel gateway, service-first Connections, real effects, governed context/routines/cloud execution, Kennel/mobile/Telegram/Discord acceptance, staging, and production.
...
- [ ] **ISC-2 — One execution writer:** #80's reviewed writer and #87's execution-environment boundary are landed, but this criterion remains open until #88 proves the public WorkUnit bridge. **Falsifier:** two writers, adapter-local canonical truth, a Coordinator bypass, or local/fake proof called live integration.
...
| **B1** | shared command primitives and WorkUnit to one trusted execution writer | completed [#81](https://github.com/Pin4sf/waldo-backend/issues/81) → landed [#80](https://github.com/Pin4sf/waldo-backend/issues/80)/[PR #125](https://github.com/Pin4sf/waldo-backend/pull/125) → landed [#87](https://github.com/Pin4sf/waldo-backend/issues/87)/[PR #126](https://github.com/Pin4sf/waldo-backend/pull/126) → active [#88](https://github.com/Pin4sf/waldo-backend/issues/88) | #81 fixtures released at `df0abae`; #80 landed at `cffae3b`; #87 landed at `9a2b11b` with reviewed head `5b3d9f5`; #88 owns only the public WorkUnit bridge and deterministic fake-consumer proof; ISC-2 and the execution half of ISC-3 pass only after #88; #86 specializes shared command primitives into the channel envelope at B3 |
...
Continue only B1 issue #88 in a new clean-worktree session:

1. fetch fresh `origin/main`, inspect #116/#78/#88, closed #80/#81/#87, merged PRs #125/#126, the #88 frontier ledger, current public ingress/route manifest, `WaldoCoordinator`, the #80 writer, #87 boundary, and all related tests; preserve unrelated dirty work and reconcile concurrent claims;
2. post `SESSION START` on #88 and #116 before writes, record exact branch/worktree/base/file ownership and planner/workflow/reviewer roster, then remove `ready-for-agent` only when actually claimed;
3. use `/waldo-isa-run-contract` or `/current-ideal-gap`, planner and workflow-mapper before implementation; use TDD, `/check-contract`, `/break-feature`, `/code-review`, mandatory Security/authority review, and independent QA/Standards/Spec review;
4. prove one owner-authenticated public WorkUnit path through the existing `RunLoopDO` composition/I/O shell, #80, and #87; allow only a bounded expected WorkUnit revision from the caller, re-read and validate the authoritative canonical revision/digest/Outcome/authority binding, and cover duplicate/replay/restart/cancel/expiry/stale-result/invalid-observation/cross-owner falsifiers, external I/O outside transactions, and no implied product closure;
5. if safe durable operation-intent reconstruction needs a new writer, migration, or released #81 contract/fixture change, post `NEEDS DIRECTION` before changing it. Do not start B2-B6, real adapters, Supabase-versus-D1, staging, deployment, or live acceptance.
