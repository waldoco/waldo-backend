...
- missing or unproved: resume/steer/pause intent durability; upgrade of WorkUnits already claimed by the legacy v0.3 planning row; every real adapter; #84 Evidence/Verification/Acceptance; #85 OpenLoop/ReEntry; multi-presence Home and channel gateway; service-first Connections; `SourceAdmissionPolicy`, automatic Personal Profile compilation, local/Waldo kernel and provider projections, universal external-LLM egress, record-scoped Kennel memory attachment; real effects; governed routines/cloud execution; Kennel/mobile/Telegram/Discord acceptance; staging; and production.
...
The backend is complete when one owner-authenticated path continuously compiles admitted work/personal understanding into correctable Profile/Memory Claims, supplies the bounded current kernel to eligible Waldo sessions, gates every external model call to a destination-specific sanitized projection, and carries responsibility from intent through bounded execution, candidate Evidence, independent Verification, conscious Acceptance/reopen/release, OpenLoop/ReEntry, and ordered projection across the three primary launch surfaces: Electron Kennel, Waldo mobile, and messaging. Telegram and Discord must pass real adapter and production acceptance; WhatsApp remains a tracked primary target but vendor approval cannot block launch. The backend must also safely perform and read back one real reversible effect, govern source admission/context/credentials/routines/workspace/attachment/deletion, and pass staging plus production operating proof without making health or agent infrastructure prerequisites.
...
- [ ] **ISC-11 — Anti:** no second writer for the same Outcome/task/Profile/Memory/execution record, no raw health in agent execution, no silent authority widening, and no deployment without explicit approval. Kennel may own pre-attachment and `local_only` records, but they are not Waldo truth and never enter cloud stores without record-scoped admission receipts.
- [ ] **ISC-12 — Admitted continuous personal understanding:** connected work/personal sources—including allowed sent mail—compile into source-bearing editable Profile Claims only after lawful/account/organization/category/processor policy admission; explicit statements/corrections outrank inference; eligible local sessions receive the current bounded kernel automatically; attachment is content-free-inventory first and one writer per record. **Falsifier:** a connect toggle overrides denial, partial coverage appears complete, an old inference resurrects, `local_only` reaches cloud, or two writers revise one record.
- [ ] **ISC-13 — Universal external-LLM egress:** every planner, executor, Brief, retrieval, semantic verifier, embedding, transcription, enrichment, provider-tool, and model-facing MCP path requires explicit destination/data-class consent, active applicable DPA, `RemoteContextEgressPolicy`, processor region/retention, and fail-closed Scribe masking; only the minimum provider projection leaves. **Falsifier:** any bypass/fallback sends the unsanitized kernel, unrelated personal/work category, PII, secret, raw health, or a call under stale/missing policy.
...
| **B4** | capability/source admission, credentials, universal external-model egress, Connections, first effects, Telegram and Discord | [#90](https://github.com/Pin4sf/waldo-backend/issues/90) → [#91](https://github.com/Pin4sf/waldo-backend/issues/91) → [#109](https://github.com/Pin4sf/waldo-backend/issues/109) + [#82](https://github.com/Pin4sf/waldo-backend/issues/82) → [#83](https://github.com/Pin4sf/waldo-backend/issues/83) → [#89](https://github.com/Pin4sf/waldo-backend/issues/89) + [#112](https://github.com/Pin4sf/waldo-backend/issues/112) + [#113](https://github.com/Pin4sf/waldo-backend/issues/113) | ISC-7 and ISC-13 pass with Calendar/inbox test accounts, organization/account/category denial, DPA/region/retention/Scribe no-bypass fixtures, and credentialed Telegram/Discord staging |
| **B5** | Personal Profile/kernel/context, record-scoped Kennel attachment, commitments, routines, cloud execution and durable messaging | [#92](https://github.com/Pin4sf/waldo-backend/issues/92), [#94](https://github.com/Pin4sf/waldo-backend/issues/94), [#110](https://github.com/Pin4sf/waldo-backend/issues/110), [#114](https://github.com/Pin4sf/waldo-backend/issues/114); new bounded Profile/attachment issue before implementation | ISC-8 and ISC-12 pass through fresh-session continuity, correction/exclusion, content-free inventory/per-item receipts, attach/detach/non-resurrection, and fault-injected staging including channel restart/rate-limit/retry cases |
| **B6** | portability/deletion and three-surface production release | [#96](https://github.com/Pin4sf/waldo-backend/issues/96) → [#111](https://github.com/Pin4sf/waldo-backend/issues/111) | ISC-9–ISC-13 pass with Electron, mobile, Telegram and Discord operational evidence and every prior source-admission/personal-understanding/egress gate still green |
...
|---|---|---|---|
| Electron Kennel | [contracts/client #26](https://github.com/Pin4sf/kennel/issues/26), [Home/Work #27](https://github.com/Pin4sf/kennel/issues/27), [B6 acceptance #28](https://github.com/Pin4sf/kennel/issues/28); standalone memory/Profile issue required | contract/client shell after B0; UI and fake-backend conformance as each fixture release lands; no-account local Profile/kernel proof may run independently against local contracts | B1/B2 client, B3 presence, B4 external-egress contract, B5 record-scoped attachment/context, B6 release |
...
| ISC-12 | source admission, Profile compiler, explicit/inferred precedence, fresh-session kernel, partial failure, attach/detach, correction/deletion/non-resurrection and baseline eval | no unlawful/org-denied/excluded source; one writer per record; measurable continuity without opaque profiling |
| ISC-13 | remote-route enumeration plus consent/DPA/region/retention/Scribe mutation tests for planner/executor/Brief/retrieval/verifier/embedding/transcription/enrichment/tool/MCP paths | no bypass, unsanitized kernel, unrelated scope, PII, secret, or raw-health egress |
...
> Backend work is paused by human scope. Do not create or resume the persistent B2–B6 goal until the human explicitly redirects work from the Kennel harness. Then re-pin `origin/main`, inspect #78/#84/#116 and PR #133, and register the root `SESSION START` before implementation or consolidation. Merge PR #133 only under separate exact-SHA authorization; after an authorized merge, compare reviewed and landed trees and run the fresh-main wall. Begin only a fresh #84 runtime lane after another explicit implementation instruction; keep #85 and B3–B6 blocked until their named barriers pass. Treat SourceAdmissionPolicy, Continuity-owned admitted Profile/Memory Claims, ContextCompiler-owned kernel/projections, record-scoped Kennel attachment, and the universal ExternalLLMEgressGate as later B4/B5 impact surfaces, not permission to widen #84. Use separate clean worktrees, keep shared contracts/migrations/schema lineages/composition roots/canonical writers single-owned, and run planning, TDD, adversarial QA, Security/authority, Standards, Spec and exact-SHA verification for every slice. Do not deploy, perform destructive drills, use production credentials, or let provider/tool success imply Waldo Acceptance.
