...
**Next action:** review [#87](https://github.com/Pin4sf/waldo-backend/issues/87)'s backend `ExecutionEnvironmentPort` PR and exact-SHA evidence; do not merge or deploy without explicit authorization, and keep [#88](https://github.com/Pin4sf/waldo-backend/issues/88) blocked until #87 lands
...
- #87 is implemented on branch `codex/87-execution-environment-port` from `origin/main@cffae3b`; runtime candidate `07636b2` defines the private trusted-intent/port/fake boundary with no contract, fixture, migration, or writer change. Focused proof is 4 files / 54 tests, the runtime wall is 42 files / 1,060 tests, and independent QA, Security, and Standards reviews pass at that exact source SHA. The final documentation-inclusive PR head, clean Linux/Docker wall, and Spec disposition are recorded in the live #87/#116 handoff. #88 remains blocked.
...
Missing or unproved: #87 landing and any real adapter/live acceptance; the #88 public WorkUnit-to-trusted-execution bridge; Judgment/Evidence/Verification/Acceptance/OpenLoop/ReEntry; multi-presence Home/channel gateway; service-first Connections and real reversible effects; governed context, routines, credentials and laptop-off execution; Kennel/mobile/Telegram/Discord acceptance; deletion/restore; staging; and production operations.
...
| B1 | shared command primitives and one WorkUnit-to-execution writer | #81, #80, #87, #88 | #81 fixtures released at `df0abae`; #80 landed at `cffae3b` with reviewed runtime `eebe931`; #87 is implemented for review on `codex/87-execution-environment-port` but not merged; #88 waits for landed #87; #86 owns the B3 channel envelope |
...
1. Read [execution ledger #116](https://github.com/Pin4sf/waldo-backend/issues/116), [#78](https://github.com/Pin4sf/waldo-backend/issues/78), the live [#87](https://github.com/Pin4sf/waldo-backend/issues/87) PR/handoff, blocked [#88](https://github.com/Pin4sf/waldo-backend/issues/88), closed [#80](https://github.com/Pin4sf/waldo-backend/issues/80), [PR #125](https://github.com/Pin4sf/waldo-backend/pull/125), closed [#81](https://github.com/Pin4sf/waldo-backend/issues/81), and the [active frontier ledger](../ledger/2026-08-14-b1-execution-environment-port-frontier.md); fetch fresh refs and preserve every unrelated dirty checkout.
2. Review only the registered #87 branch and PR. Verify base/head/remote equality, complete exact-SHA wall, independent review dispositions, and any new review comments before changing source.
3. Preserve `WaldoCoordinator` and #80 as the only public authority/sole writer, the private stable operation-intent seam, read-only recovery before effectful issue, strict adapter/environment fencing, and provider/environment category separation.
4. If production routing cannot durably supply the same operation intent after restart without a new writer, migration, or released-contract change, post `NEEDS DIRECTION`; do not hide the gap in adapter state or expand #87 into #88.
5. Do not merge or deploy without explicit authorization. Keep #88 blocked and do not start B2-B6, the Supabase/D1 decision, staging, real Kennel/cloud/local adapters, or live consumer acceptance in #87.

### Copy-ready next-session prompt

> Review Waldo backend B1 issue #87 from the live PR branched at `origin/main@cffae3b`. Read `AGENTS.md`, `.claude/rules/INDEX.md`, the foundation entrypoints, `docs/ledger/2026-08-14-b1-execution-environment-port-frontier.md`, GitHub issues #116/#78/#87, blocked #88, closed #80/#81, merged PR #125, and every current PR review/check. Verify the exact local/remote head and the clean Linux/Docker wall before changing anything. Preserve `WaldoCoordinator` as the public authority boundary and #80 as sole writer; preserve the non-caller-accessible trusted operation intent, recover-before-issue, target-side fencing, provider/environment distinction, and non-self-certifying observation boundary. If durable production intent reconstruction needs a new writer, migration, or released contract, post `NEEDS DIRECTION`. Do not merge or deploy without explicit authorization; do not start #88/B2-B6, change contracts/fixtures, add real adapters, decide Supabase versus D1, or claim live acceptance.
