...
**Next action:** implement only [#88](https://github.com/Pin4sf/waldo-backend/issues/88)'s public owner-authenticated WorkUnit-to-trusted-execution bridge around the landed #80 sole writer and #87 execution-environment boundary; do not start B2-B6, real adapters, deployment, or live acceptance
...
Checked 2026-08-14 after the authorized #87 merge:

- current implementation pin: `origin/main@9a2b11b`; B0 implementation landed in [PR #119](https://github.com/Pin4sf/waldo-backend/pull/119), test isolation in [PR #121](https://github.com/Pin4sf/waldo-backend/pull/121), additive v0.4 responsibility contracts/fixtures in [PR #123](https://github.com/Pin4sf/waldo-backend/pull/123), their handoff in [PR #124](https://github.com/Pin4sf/waldo-backend/pull/124), #80's sole writer in [PR #125](https://github.com/Pin4sf/waldo-backend/pull/125), and #87's execution-environment boundary in [PR #126](https://github.com/Pin4sf/waldo-backend/pull/126);
...
| B1 | shared command primitives and one WorkUnit-to-execution writer | #81, #80, #87, #88 | #81 fixtures released at `df0abae`; #80 landed at `cffae3b` with reviewed runtime `eebe931`; #87 landed at `9a2b11b` with reviewed head `5b3d9f5`; #88 is the only active B1 implementation frontier; #86 owns the B3 channel envelope |
...
1. Start #88 only in a fresh clean worktree from current `origin/main`; verify the live issue, dependency pins, consumers, related PRs, and absence of a concurrent owner before writing. Read [#116](https://github.com/Pin4sf/waldo-backend/issues/116), [#78](https://github.com/Pin4sf/waldo-backend/issues/78), closed [#80](https://github.com/Pin4sf/waldo-backend/issues/80), closed [#81](https://github.com/Pin4sf/waldo-backend/issues/81), closed [#87](https://github.com/Pin4sf/waldo-backend/issues/87), merged PRs #125/#126, and the [#88 frontier ledger](../ledger/2026-08-14-b1-workunit-execution-bridge-frontier.md).
2. Post `SESSION START` on #88 and #116 before writes; claim exact file/module ownership, remove `ready-for-agent` only when the worktree is actually owned, and use `/waldo-isa-run-contract` or `/current-ideal-gap`, planner, and workflow-mapper before implementation.
3. Map the complete public Worker/gateway → owner Durable Object → `WaldoCoordinator` → #80 committed authority → trusted operation-intent resolution → existing `RunLoopDO` composition/I/O shell → #87 recover/execute → public Coordinator admission path. Keep external I/O outside SQLite transactions and preserve one retry owner.
4. Permit only a bounded caller-supplied expected WorkUnit revision as a concurrency precondition. Re-read canonical WorkUnit state and derive/validate the authoritative revision, digest, Outcome binding, owner, authority ceiling, execution binding, context, operation intent, provider, and environment server-side; reject attempts to supply or substitute those authoritative fields. Provider/executor completion remains an observation and cannot change Outcome, Evidence, Verification, Acceptance, or OpenLoop truth.
5. If safe replay/restart requires a new canonical operation-intent writer, migration, or a change to released #81 contracts/fixtures, post `NEEDS DIRECTION` with evidence before changing them. Do not start B2-B6, real adapters, Supabase-versus-D1, staging, deployment, or live acceptance in #88.

### Copy-ready next-session prompt

> Continue Waldo backend production convergence as the new clean-worktree owner for B1 issue #88, the public WorkUnit-to-trusted-execution bridge. Fetch fresh `origin/main`; live state outranks this prompt. Read `AGENTS.md`, `.claude/rules/INDEX.md` and every required rule, the foundation entrypoints, architecture lock, `docs/ledger/2026-08-14-b1-workunit-execution-bridge-frontier.md`, closed #80/#81/#87, merged PRs #125/#126, and GitHub issues #116/#78/#88. Begin read-only; inspect concurrent claims, post `SESSION START` on #88/#116, and use `/waldo-isa-run-contract` or `/current-ideal-gap`, planner, and workflow-mapper before writing. Implement only one authenticated public path through `WaldoCoordinator`, the #80 sole writer, the existing `RunLoopDO` composition/I/O shell, and the #87 execution-environment boundary. The caller may supply only a bounded expected WorkUnit revision as a concurrency precondition; re-read canonical state and derive/validate the authoritative revision, digest, Outcome binding, owner, authority, binding, provider, environment, context, and operation intent server-side. Perform external I/O outside SQLite transactions; reconcile ambiguity before reissue; reject invalid observations; and keep executor completion non-self-certifying. If this needs a new canonical intent writer, migration, or released #81 contract/fixture change, post `NEEDS DIRECTION` before changing it. Use TDD, `/check-contract`, `/break-feature`, `/code-review`, mandatory Security review, and the full exact-SHA wall. Do not start B2-B6, add real adapters, decide Supabase versus D1, deploy, or claim live/product acceptance.
