...
3. Map the complete public Worker/gateway → owner Durable Object → `WaldoCoordinator` → #80 committed authority → trusted operation-intent resolution → #87 recover/execute → public Coordinator admission path. Keep external I/O outside SQLite transactions and preserve one retry owner.
4. Derive owner, Outcome/WorkUnit revision and digest, authority ceiling, execution binding, context, operation intent, provider, and environment from authenticated/canonical state. Reject caller attempts to supply or substitute them. Provider/executor completion remains an observation and cannot change Outcome, Evidence, Verification, Acceptance, or OpenLoop truth.
