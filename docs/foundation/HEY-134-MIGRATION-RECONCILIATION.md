...
| Consent | Legacy boolean/policy-version audit columns | ADR-0073 requires class/source/purpose/version/status/timestamps/18+ evidence | Add nullable canonical columns without fabricating legacy evidence; enforce all new/changed records; active consent gates health writes by source and purpose |
...
| CRS persistence | Historical columns lacked contract checks | Contract fixes score, zone thresholds, and confidence bounds | Add score, zone, score/zone agreement, and confidence checks only |
