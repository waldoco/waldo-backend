...
the HEY-12 PreLLMCall/PostLLMCall hooks, and return only bounded model text plus metering. Keep
HEY-136 run-loop integration, Scribe memory writes, memory/context hydration, live provider calls,
credentials, and channel delivery out of scope. After HEY-17 lands, the next runtime-lane unit is
HEY-136 SLICE-6 run-loop integration with fake-backed provider/context/channel seams. Use
