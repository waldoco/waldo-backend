...
2. Gmail read (regression for the batch-overflow break): "any new email?" → get_communication; a populated inbox must render items, never a false "nothing found". (Real provider data; content-free report.)
3. Tasks read: "what are my open tasks" → get_tasks typed result. First live run of this tool. (Real provider data; content-free report.)
