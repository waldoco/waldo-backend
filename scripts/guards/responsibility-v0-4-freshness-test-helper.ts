...
import { expect } from 'vitest'; import { findFixtureBundleDrift } from './fixture-bundle-freshness';
export function assertResponsibilityV04BundleFresh(directory: URL, build: (hash: (value: string) => string) => Record<string, string>): void {
  ...
  try {
    for (const [path, contents] of Object.entries(bundle)) { mkdirSync(scratch, { recursive: true }); writeFileSync(join(scratch, path), contents); }
    const target = Object.keys(bundle).sort()[0]!; writeFileSync(join(scratch, target), bundle[target]!.replace(/\n/g, '\r\n'));
