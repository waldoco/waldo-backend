...
-- Claim an intent: 'new' for the first caller, 'done' (+ stored result) or 'pending' for a
-- replay. Owner-gated exactly like proxy_access.
create function waldo.proxy_idem_claim(p_do_name text, p_connection uuid, p_key text) returns jsonb
language plpgsql security definer set search_path = '' as $$
declare v_owner uuid; v_status text; v_result jsonb;
begin
  v_owner := waldo.owner_id_for(p_do_name);
  if v_owner is null then return null; end if;
  insert into waldo.proxy_idempotency (owner_id, connection, pkey) values (v_owner, p_connection, p_key)
    on conflict (owner_id, connection, pkey) do nothing;
  if found then return jsonb_build_object('state', 'new'); end if;
  select status, result into v_status, v_result from waldo.proxy_idempotency
...
revoke all on function waldo.proxy_idem_claim(text, uuid, text) from public, anon, authenticated;
revoke all on function waldo.proxy_idem_store(text, uuid, text, jsonb) from public, anon, authenticated;
grant execute on function waldo.proxy_idem_claim(text, uuid, text) to service_role;
