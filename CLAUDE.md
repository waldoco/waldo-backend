...
Most of `waldo-backend` lives in Codex's cluster (runtime + infra + delivery). Claude owns Supabase schema (HEY-9), the CRS engine (HEY-102), memory/Scribe/recall, and the GDPR erasure runbook.
...
**Agent-Ready bar (10 items):** SoT links · Module/Interface/Seam · deps · acceptance (golden test) · validation commands · 5-bucket failure paths (null / permission-revoked / network / hostile / concurrent) · reversibility · security/privacy · out-of-scope · zero open founder/legal questions. Missing any → `needs-info` or `ready-for-human`.
